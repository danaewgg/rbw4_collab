...
- If a tab is already open, it'll go to it instead of doing nothing
- Removed MercurySettings.json, I'll replace it with an actual config system at some point
- Replaced some library assets (internal icons, tooltips, etc)
- Changed some parameters' defaults
- Replaced themes
- Renamed MercuryUI global to UILoaded
- Made themes' section height to be dynamic, scales with the amount of themes (it needs to adjust based on the library size too)
...
local Library = {
	Themes = {
        ...
        ["Dan's Hub"] = {
            Main = Color3.fromRGB(70, 90, 132),
            Secondary = Color3.fromRGB(59, 77, 113),
            Tertiary = Color3.fromRGB(195, 200, 214),
            StrongText = Color3.fromRGB(255, 255, 255),
            WeakText = Color3.fromRGB(175, 183, 203),
        },
		["Opera GX"] = {
            Main = Color3.fromRGB(18, 18, 18),
            Secondary = Color3.fromRGB(34, 34, 34),
            Tertiary = Color3.fromRGB(200, 0, 0),
        ...
        ["Visual Studio"] = {
            ...
            Tertiary = Color3.fromRGB(169, 123, 223),
            StrongText = Color3.fromRGB(255, 255, 255),
...
function Library:create(options)
	-- I removed MercurySettings.json


	options = self:set_defaults({
		Name = "Home",
		Size = UDim2.fromOffset(600, 400),
		Theme = self.Themes.Mercury,
		Link = "https://github.com/deeeity/mercury-lib"
	}, options)

	if getgenv and getgenv().UILoaded then
		UILoaded()
		UILoaded = nil
	end
	...
	local homeButtonIcon = homeButton:object("ImageLabel", {
		...
		Image = "http://www.roblox.com/asset/?id=12858920241", -- Mercury default: 8569322835
		Theme = {ImageColor3 = "StrongText"}
	})
	...
	settingsTab:slider{
		Name = "Drag Speed",
		Description = "How smooth the UI dragging looks",
		Max = 20,
		Default = 14,
	...
	creditsTab:credit{Name = "UI Library", Description = "Helped make this possible", Github="https://github.com/deeeity/mercury-lib/blob/master/src.lua"}
...
function Library:notification(options)
	options = self:set_defaults({
		...
		Text = [[Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut
			labore et dolore magna aliqua. Nunc id cursus metus aliquam eleifend.]],
		Duration = 3,
...
function Library:dropdown(options)
	...
	function methods:AddItems(fitems)
		...
		for i, item in next, items do
			...
			if typeof(label) == "table" then continue end
...
function Library:_theme_selector()
	...
	local themeContainer = self.container:object("Frame", {
		Theme = {BackgroundColor3 = "Secondary"},
		Size = UDim2.new(1, -20, 0, 35 + 83 * math.ceil(themesCount / 5)), -- adjust height based on number of themes
	}):round(7)
	...
	for themeName, themeColors in next, Library.Themes do
		...
		for _, color in next, themeColors do
			if not (typeof(color) == "boolean") then
				count += 1
			end
		end
			...
			if listening and not UserInputService:GetFocusedTextBox() then
				if key.UserInputType == Enum.UserInputType.Keyboard then
					if key.KeyCode ~= Enum.KeyCode.Escape then
						...
						options.OnBindChanged(key.KeyCode)
					end
	...
	do
		...
		sliderContainer.MouseButton1Down:connect(function()
			...
			while RunService.RenderStepped:wait() and down do
				...
				local value = math.floor(((options.Max - options.Min) * percentage) / options.Step + 0.5) * options.Step + options.Min

				valueText.Text = tostring(math.round(value * 10) / 10)
